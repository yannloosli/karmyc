...
export const predefinedRules = {
  ...
  // Règle pour les actions de projet
  project: {
    createProject: {
      actionType: 'project/createProject',
      validator: (action: AnyAction) => {
        if (!action.payload?.name) {
          return { valid: false, message: 'Nom du projet requis' };
        }
        return { valid: true };
      },
      message: 'Validation de la création de projet'
    }
  }
}; 
