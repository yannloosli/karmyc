...
export class ActionSecurity {
  ...
  registerPermission(permission: IActionPermission): void {
    ...
    actionLogger.info(`Permission enregistrée pour l'action: ${permission.actionType}`);
  }

  unregisterPermission(actionType: string): void {
    this.permissions.delete(actionType);
    actionLogger.info(`Permission supprimée pour l'action: ${actionType}`);
  }
  ...
  validateAction(action: AnyAction, userPermissions: Permission[]): boolean {
    ...
    if (!hasRequiredPermissions) {
      actionLogger.warn('Action rejetée - Permissions insuffisantes', action, {
    ...
    if (permission.rateLimit) {
      ...
      if (isRateLimited) {
        actionLogger.warn('Action rejetée - Rate limit dépassé', action, {
