...
/**
 * Configuration du plugin de performance
 */
export interface IPerformanceConfig {
  enabled: boolean;
  maxMetrics: number;
  slowActionThreshold: number; // en millisecondes
  actionTypesToTrack?: string[];
}

/**
 * Métriques de performance pour une action
 */
...
class PerformanceMonitor {
  ...
  endTracking(action: AnyAction, success: boolean = true): void {
    ...
    // Avertir si le temps d'exécution est trop long
    if (executionTime > this.config.slowActionThreshold) {
      actionLogger.warn('Action lente détectée', action, {
        executionTime,
        actionType: action.type
      });
    }
...
/**
 * Plugin de performance pour les actions
 * Permet de mesurer et d'analyser les performances des actions
 */
...
// Export pour l'accès au moniteur de performance
