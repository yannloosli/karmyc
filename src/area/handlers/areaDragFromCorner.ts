...
export const handleAreaDragFromCorner = (
    ...
    requestAction({}, (params) => {
        ...
        if (parts.length !== 2 || !parts.every(part => ["n", "s", "e", "w"].includes(part))) {
            throw new Error(`Invalid corner direction: ${corner}`);
        }
        const directionParts = parts as [CardinalDirection, CardinalDirection];

        // onMoveFn is set to handle mouse move events after the mouse has moved
        // in a specific direction.
        let onMoveFn: ((mousePosition: Vec2) => void) | undefined;
